...
-- instance YulO2 b r =>
--          UncurriableTupleN '[] b (YulCat'P r) (YulCat'P r) Many (YulCat'P r) (YulCat'P r) Many where
--   uncurryN b _ = b
...
instance forall x xs b r.
  ...
  curryNP fNP x = curryNP @xs @b @(YulCat'P r) @(YulCat'P r) @_ @(YulCat'P r) @_ (fNP . cons_NP x)
...
instance forall f xs b r.
         ...
         , TraversableNP (YulCat'P r) xs
         ) =>
         CallableFunctionN PureFn f xs b (YulCat'P r) (YulCat'P r) Many where
  callN (MkPureFn (cid, cat)) tpl = sequence_NP (fromTupleNtoNP tpl) >.> YulJmpU (cid, cat)
